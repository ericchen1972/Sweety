...
回覆語言：
- msg_reply 必須使用對方最新一則可辨識實質文字訊息的主要語言；對方用日文就以自然日文回覆，用英文就以英文回覆，用中文就用中文。App 介面語言與人設說明使用的語言都不能取代這項判斷。
...
- 對方傳來的影片、語音或其他音訊都算對方新訊息；使用簡短客觀描述，畫面看得到長度時一併記錄。
...
請先根據最近對話與總筆數判斷雙方熟稔度，再決定語氣、長度、是否反問、是否延後或是否保留。', 1)
ON DUPLICATE KEY UPDATE
